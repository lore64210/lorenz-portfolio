...
export default () => {
    ...
    const setup = (p5) => {
        nodes = Array.from(Array(NODES_AMOUNT).keys()).map(() =>
            p5.createVector(
                p5.random(width / (POPULATION_AMOUNT <= 10 ? 2 : 1)),
                p5.random(height)
            )
        );
        population = new Population(
            nodes.map((val, indx) => indx),
            POPULATION_AMOUNT,
            width * height,
            MUTATION_RATE
        );
...
const shuffle = (originalArray) => {
    ...
    for (let i = 0; i < array.length - 1; i++) {
        const index = Math.ceil(
            Math.random(array.length - 1) * array.length - 1
        );
        const temp = array[i];
        array[i] = array[index];
        array[index] = temp;
    ...
    pick(parent) {
        ...
        while (!found) {
            const selectedIndex = Math.round(
                Math.random() * (this.population.length - 1)
            );
...
class DNA {
    ...
    crossOver(partner) {
        ...
        const middleIndex = Math.ceil(
            this.genotype.length / 2 + this.genotype.length / 4
        );
